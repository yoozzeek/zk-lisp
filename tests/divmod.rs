...
fn divmod_q_r_e2e_small_cases() {
    ...
    let pi_q = pi::PublicInputsBuilder::from_program(&program_q)
        .build()
        .expect("pi q");
    let trace_q = build_trace(&program_q, &pi_q).expect("trace q");

    let cols = Columns::baseline();
    let (out_reg_q, out_row_q) = zk_lisp::vm_output_from_trace(&trace_q);
    ...
    let pi_r = pi::PublicInputsBuilder::from_program(&program_r)
        .build()
        .expect("pi r");

    let trace_r = build_trace(&program_r, &pi_r).expect("trace r");
    let (out_reg_r, out_row_r) = zk_lisp::vm_output_from_trace(&trace_r);
