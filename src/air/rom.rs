...
impl AirModule for RomAir {
    ...
    fn append_assertions(
        ...
        // Derive on-the-fly from Blake3
        // if fields were not populated.
        let (f0, f1) = if ctx.pub_inputs.program_commitment_f0 == BE::ZERO
            && ctx.pub_inputs.program_commitment.iter().any(|b| *b != 0)
        {
            let fc = program_field_commitment(&ctx.pub_inputs.program_commitment);
            (fc[0], fc[1])
        } else {
            (
                ctx.pub_inputs.program_commitment_f0,
                ctx.pub_inputs.program_commitment_f1,
            )
        };
